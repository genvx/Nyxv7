	...
	vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				repeat task.wait(0.08) until game:IsLoaded()
        ...
        while pollingActive do
            ...
            for _, u in ipairs(data.users) do
                ...
                if uid and uid ~= lplr.UserId then
                    ...
                    if playerInServer then
                        ...
                        if shouldShow then
                            newMap[uid] = {tier = utier, username = u.username or '?',istiernone = tiernone}
                        end
                    end
                end
            end
            ...
            for uid, info in pairs(newMap) do
                if not prev[uid] then
					if info.istiernone then
						vape:CreateNotification('[AEROV4] Injected', string.format('[T%d] %s injected', 0, info.username), 6)
					else
            ...
            for uid, info in pairs(prev) do
                if not newMap[uid] then
					if info.istiernone then
						vape:CreateNotification('[AEROV4] Injected', string.format('[T%d] %s uninjected', 0, info.username), 6)
					else
						vape:CreateNotification('[AEROV4] Injected', string.format('[T%d] %s uninjected', info.tier, info.username), 6)
					end
