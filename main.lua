...
do
    ...
    pcall(function()
        ...
        if _c1 and _c2 and debug and debug.info then
    ...
    if _hookDetected then
        game:GetService('Players').LocalPlayer:Kick('[AEROV4] integrity check failed')
        error('[AEROV4] loadstring hook detected', 2)
    end
end
    ...
    if not isfile(path) then
        ...
        for attempt = 1, 3 do
            local suc, result = pcall(function()
                return game:HttpGet('https://raw.githubusercontent.com/genvx/Nyxv7/' .. readfile('newvape/profiles/commit.txt') .. '/' .. select(1, path:gsub('newvape/', '')), true)
            end)
    ...
    vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
        if (not teleportedServers) and (not shared.VapeIndependent) then
            ...
            local teleportScript = [[
                loadstring(game:HttpGet('https://raw.githubusercontent.com/genvx/Nyxv7/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
            ]]
...
vape = guiFunc()
if not vape then
    error('[AEROV4] GUI returned nil')
end

shared.vape = vape
task.wait(0.1)

-- Simplified Tier (no whitelist/website)
getgenv().getAeroTier = function() return 99 end
    ...
    if isfile('newvape/games/' .. gameFileId .. '.lua') then
        ...
        if not shared.VapeDeveloper then
            local suc, res = pcall(function()
                return game:HttpGet('https://raw.githubusercontent.com/genvx/Nyxv7/' .. readfile('newvape/profiles/commit.txt') .. '/games/' .. gameFileId .. '.lua', true)
            end)
